...

  {
    ...
    findings: [

      {
        ...
        disclosure: [
          { date: "2026-08-13 20:33", event: "Found KillWorldRequest registered on the server side with no permission check, while reading through ChunkPregenerator's packet registrations" },
          { date: "2026-08-14 19:15", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 17:11", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 19:58", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "Should check `player.isCreative() || player.canUseCommand(2, \"pregen\")` (or\n`PregenAPI.hasPermission`) before acting. No packet handler in ChunkPregenerator calls\n`isOpped` / permission-level / `PregenAPI.hasPermission` - the only permission reference is\nin `onPlayerLoggedIn` (auto-restart tasks for ops), not in any packet path.\n\nThe shape of the guard:\n\n```\n// KillWorldRequest, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(KillWorldRequest msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the authority check this handler never makes\n        if (!(player.isCreative() || player.canUseCommand(2, \"pregen\"))) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        disclosure: [
          { date: "2026-08-10 22:07", event: "Found SyncItemDataPacket registered on the server side with no permission check, while reading through Trinkets & Baubles's packet registrations" },
        ...
        credit: "Credited as \"KL BanYourself\" in Trinkets & Baubles 0.33.4, released 2026-08-21. The same commit hardened SyncItemDataPacket, the handler in this finding, so the credit and the fix are one change rather than two. Verify on the [CurseForge release](https://www.curseforge.com/minecraft/mc-mods/trinkets-and-baubles/files/8703456) or in [commit e07d279](https://github.com/XzeroAir/Trinkets/commit/e07d279901cbf64c85d1adee7dd7aa60284a840a).\n\n![The commit adding KL BanYourself to the mod credits in mcmod.info](assets/img/trinkets-credit-github.webp)\n\n![CurseForge release notes for 0.33.4, crediting the packet exploit report and hardening the network packets](assets/img/trinkets-credit-curseforge.webp)\n\n![The maintainer's reply the same day the report was sent](assets/img/trinkets-credit-discord.webp)",
...

      {
        ...
        disclosure: [
          { date: "2026-08-14 20:08", event: "Found PacketEditTileEntity registered on the server side with no permission check, while reading through RecurrentComplex's packet registrations" },
          { date: "2026-08-14 20:58", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 12:54", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 13:54", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "`PacketEditTileEntityHandler.processServer` never calls\n`RecurrentComplex.checkPerms(player)` (unlike the gated handlers). Should gate on\n`player.canUseCommand(2, \"setblock\")` before calling `readSyncedNBT`, and validate the tile\nis within the sender's reach. `SpawnCommandLogic.canUseCommand` should also be tightened\nbelow level 2.\n\nThe shape of the guard:\n\n```\n// PacketEditTileEntity, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketEditTileEntity msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the authority check this handler never makes\n        if (!(RecurrentComplex.checkPerms(player))) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        disclosure: [
          { date: "2026-08-13 17:05", event: "Found PlayerMovementMessage registered on the server side with no permission check, while reading through Grappling Hook (grapplemod)'s packet registrations" },
          { date: "2026-08-13 18:13", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-14 18:20", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-14 20:35", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "The server should validate the target position is reachable/legitimate (e.g. within a\ngrapple range or a server-side movement check) rather than trusting the client's arbitrary\ncoordinates. Currently the client can teleport itself anywhere.\n\nThe shape of the guard:\n\n```\n// PlayerMovementMessage, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PlayerMovementMessage msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // The packet should carry intent, not a result. Anything a client could\n        // inflate (speed, accel, cooldown) gets recomputed here from server state\n        // rather than read straight out of msg.\n        if (!withinServerLimits(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "2.19a",
        cwe: "CWE-862 Missing Authorization",
        title: "EmptyTankPacket - drains any multiblock's fluid tanks by position",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 18:18", event: "Found EmptyTankPacket registered on the server side with no permission check, while reading through NuclearCraft's packet registrations" },
          { date: "2026-08-14 20:44", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 14:21", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 16:35", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No distance/ownership check on the BlockPos; a client can drain any player's multiblock\nreactor.\n\nThe shape of the guard:\n\n```\n// EmptyTankPacket, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(EmptyTankPacket msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // a client can name any position, so bound it to arm's length\n        if (player.getDistanceSq(msg.pos) > 36.0D) return;   // 6 blocks, squared\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.7.1",
        cwe: "CWE-862 Missing Authorization",
        title: "MessageDragonArmor - equips arbitrary armor on any dragon by ID",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-12 19:51", event: "Found MessageDragonArmor registered on the server side with no permission check, while reading through Ice and Fire's packet registrations" },
          { date: "2026-08-13 13:39", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-14 17:59", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-14 19:13", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "Should verify the dragon is owned by the sender (`isOwner(player)`) before equipping armor.\nCurrently any client can equip/change armor on any dragon.\n\nThe shape of the guard:\n\n```\n// MessageDragonArmor, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessageDragonArmor msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the sender has to own the entity it is acting on\n        Entity target = player.world.getEntityByID(msg.entityId);\n        if (!(target instanceof IOwnable) || !((IOwnable) target).isOwner(player)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "r1.6-179",
        cwe: "CWE-862 Missing Authorization",
        title: "MessageRequestEmote - broadcasts an emote to every player, and reads a server file",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 19:55", event: "Found MessageRequestEmote registered on the server side with no permission check, while reading through Quark's packet registrations" },
          { date: "2026-08-13 20:46", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-14 15:15", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-14 16:19", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "Should validate the emoteName is a known emote and that the emote-commands feature is\nintended to be client-triggerable. Currently any emoteName is accepted and the corresponding\n`.mcfunction` is executed.\n\nThe shape of the guard:\n\n```\n// MessageRequestEmote, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessageRequestEmote msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the id arrives off the wire, so it is an allowlist or nothing\n        if (!ALLOWED_IDS.contains(msg.id)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "3.19.5",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketButtonID - fires any LogicController button by position",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-12 22:34", event: "Found PacketButtonID registered on the server side with no permission check, while reading through RebornCore's packet registrations" },
          { date: "2026-08-13 19:49", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-14 13:03", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-14 15:26", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "Should verify the tile is within the sender's interaction range before triggering the\nbutton. Currently any LogicController can be triggered from anywhere.\n\nThe shape of the guard:\n\n```\n// PacketButtonID, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketButtonID msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.13.0",
        cwe: "CWE-862 Missing Authorization",
        title: "MessageDodge - dodge impulse on demand, no cooldown",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 20:53", event: "Found MessageDodge registered on the server side with no permission check, while reading through Reskillable's packet registrations" },
          { date: "2026-08-14 20:14", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 09:51", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 12:07", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "The server should enforce the dodge cooldown server-side rather than trusting the client to\nsend the packet. Currently any client can dodge at will.\n\nThe shape of the guard:\n\n```\n// MessageDodge, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessageDodge msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // The packet should carry intent, not a result. Anything a client could\n        // inflate (speed, accel, cooldown) gets recomputed here from server state\n        // rather than read straight out of msg.\n        if (!withinServerLimits(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.12.2",
        cwe: "CWE-862 Missing Authorization",
        title: "TRADE_ACCEPT - advances a trade and swaps the craft matrices",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-12 23:17", event: "Found TRADE_ACCEPT registered on the server side with no permission check, while reading through Varied Commodities's packet registrations" },
          { date: "2026-08-13 16:52", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-14 18:06", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-14 21:22", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "Should verify the trade is fully offered and both players are in the trade container before\naccepting. Currently the client can drive the trade state machine.\n\nThe shape of the guard:\n\n```\n// TRADE_ACCEPT, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(TRADE_ACCEPT msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.3-32",
        cwe: "CWE-862 Missing Authorization",
        title: "MessageDropIn - drop-in handler runs on a client-supplied slot and item",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-12 19:32", event: "Found MessageDropIn registered on the server side with no permission check, while reading through AutoRegLib's packet registrations" },
          { date: "2026-08-13 20:28", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-14 11:54", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-14 12:53", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "Should validate the `slot` is within the sender's open container bounds and the `stack` is a\nlegitimate item the sender actually holds. No such validation exists.\n\nThe shape of the guard:\n\n```\n// MessageDropIn, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessageDropIn msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the id arrives off the wire, so it is an allowlist or nothing\n        if (!ALLOWED_IDS.contains(msg.id)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.4.37",
        cwe: "CWE-862 Missing Authorization",
        title: "DropPacket - client sets a global static drop power",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 21:56", event: "Found DropPacket registered on the server side with no permission check, while reading through ItemPhysic's packet registrations" },
          { date: "2026-08-14 15:44", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 17:37", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 19:45", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "Should validate the power is within a sane range. Currently any int is accepted and stored\nglobally.\n\nThe shape of the guard:\n\n```\n// DropPacket, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(DropPacket msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the id arrives off the wire, so it is an allowlist or nothing\n        if (!ALLOWED_IDS.contains(msg.id)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.1.23",
        cwe: "CWE-862 Missing Authorization",
        title: "SkillsPacket - sets your own skill levels from client data",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 20:22", event: "Found SkillsPacket registered on the server side with no permission check, while reading through Level Up! 2's packet registrations" },
          { date: "2026-08-14 14:20", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 11:54", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 12:56", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "The server should validate the skill levels against the player's actual earned XP/level\nrather than trusting the client-supplied `data` and `levelSpend`. Currently the client can\nset arbitrary skill levels.\n\nThe shape of the guard:\n\n```\n// SkillsPacket, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(SkillsPacket msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // The packet should carry intent, not a result. Anything a client could\n        // inflate (speed, accel, cooldown) gets recomputed here from server state\n        // rather than read straight out of msg.\n        if (!withinServerLimits(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "2.0.8",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketMainhandAttack - attacks any entity by ID with client-supplied motion",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 22:16", event: "Found PacketMainhandAttack registered on the server side with no permission check, while reading through RLCombat's packet registrations" },
          { date: "2026-08-14 13:22", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 14:25", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 15:00", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "Should verify the target is within the player's reach before attacking. Currently the\nprimary target is attacked by ID with no distance check.\n\nThe shape of the guard:\n\n```\n// PacketMainhandAttack, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketMainhandAttack msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // a client can name any position, so bound it to arm's length\n        if (player.getDistanceSq(msg.pos) > 36.0D) return;   // 6 blocks, squared\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.5.3",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketLongReachAttack - attacks any entity by ID within weapon reach",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 21:53", event: "Found PacketLongReachAttack registered on the server side with no permission check, while reading through SpartanWeaponry's packet registrations" },
          { date: "2026-08-14 16:47", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 16:02", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 19:13", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "The reach check is present (good), but the velocity is stored client-supplied and the reach\nproperty itself is client-influenced via the weapon. Should validate the velocity and ensure\nthe reach value is server-authoritative.\n\nThe shape of the guard:\n\n```\n// PacketLongReachAttack, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketLongReachAttack msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // a client can name any position, so bound it to arm's length\n        if (player.getDistanceSq(msg.pos) > 36.0D) return;   // 6 blocks, squared\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.2.2",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketCollisionS - client-chosen acceleration triggers self damage",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 22:33", event: "Found PacketCollisionS registered on the server side with no permission check, while reading through CollisionDamage's packet registrations" },
          { date: "2026-08-14 13:13", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 15:33", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 18:34", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "The server should verify the accel value is plausible (e.g. derived from actual server-side\nmotion), not trust a client-supplied double. Currently the client fully controls the damage\namount.\n\nThe shape of the guard:\n\n```\n// PacketCollisionS, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketCollisionS msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // The packet should carry intent, not a result. Anything a client could\n        // inflate (speed, accel, cooldown) gets recomputed here from server state\n        // rather than read straight out of msg.\n        if (!withinServerLimits(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "2.1",
        cwe: "CWE-862 Missing Authorization",
        title: "SDodgeMessage - dodge with a client-supplied direction and cooldown",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 18:47", event: "Found SDodgeMessage registered on the server side with no permission check, while reading through ElenaiDodge's packet registrations" },
          { date: "2026-08-14 19:37", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 09:13", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 09:48", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "The server should validate the direction string against the allowed enum and enforce the\ncooldown server-side rather than trusting the client-supplied `cooldown`. Currently\n`Direction.valueOf(message.dir)` will throw on an invalid string (crash/DoS) and the\ncooldown is client-controlled.\n\nThe shape of the guard:\n\n```\n// SDodgeMessage, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(SDodgeMessage msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // The packet should carry intent, not a result. Anything a client could\n        // inflate (speed, accel, cooldown) gets recomputed here from server state\n        // rather than read straight out of msg.\n        if (!withinServerLimits(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.12.2.047",
        cwe: "CWE-862 Missing Authorization",
        title: "ControlEventPacket - posts arbitrary control events on the Forge bus",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 18:27", event: "Found ControlEventPacket registered on the server side with no permission check, while reading through FantasticLib's packet registrations" },
          { date: "2026-08-14 20:57", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 09:41", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 10:46", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No validation that the control event is one the sender is allowed to send, or that the\nidentifier/name is legitimate. Should whitelist allowed control events.\n\nThe shape of the guard:\n\n```\n// ControlEventPacket, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(ControlEventPacket msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the id arrives off the wire, so it is an allowlist or nothing\n        if (!ALLOWED_IDS.contains(msg.id)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.4.2",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketMountSpecial - fires 8 fireballs from any entity by ID",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 19:43", event: "Found PacketMountSpecial registered on the server side with no permission check, while reading through Fish's Undead Rising's packet registrations" },
          { date: "2026-08-14 12:42", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 16:39", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 17:26", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No check that the entity is the sender's own mount - the handler should verify the target\nentity is the sender's ridden/tamed mount before spawning projectiles.\n\nThe shape of the guard:\n\n```\n// PacketMountSpecial, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketMountSpecial msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the sender may only act on itself, never on an arbitrary entity id\n        if (msg.entityId != player.getEntityId()) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.64",
        cwe: "CWE-862 Missing Authorization",
        title: "ITPacketClick - arbitrary slotClick on your open container",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 17:37", event: "Found ITPacketClick registered on the server side with no permission check, while reading through InventoryTweaks's packet registrations" },
          { date: "2026-08-14 18:31", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 12:12", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 13:21", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "The handler only checks the window id matches the sender's open container; it should also\nvalidate the slot is within the container bounds and the click is legitimate. Currently a\nmalformed slot/action could cause issues.\n\nThe shape of the guard:\n\n```\n// ITPacketClick, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(ITPacketClick msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the id arrives off the wire, so it is an allowlist or nothing\n        if (!ALLOWED_IDS.contains(msg.id)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.12.2",
        cwe: "CWE-862 Missing Authorization",
        title: "PartialBlockPacket - block-break progress written by username",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 18:15", event: "Found PartialBlockPacket registered on the server side with no permission check, while reading through MultiMine's packet registrations" },
          { date: "2026-08-14 20:45", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 18:45", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 21:49", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "Should verify the block is within the sender's reach and that the sender is actually mining\nit. Currently any position can be progressed to completion and broken.\n\nThe shape of the guard:\n\n```\n// PartialBlockPacket, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PartialBlockPacket msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // a client can name any position, so bound it to arm's length\n        if (player.getDistanceSq(msg.pos) > 36.0D) return;   // 6 blocks, squared\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.9",
        cwe: "CWE-862 Missing Authorization",
        title: "CToSMessage - zeroes fall velocity, or uses an entity by ID",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 17:20", event: "Found CToSMessage registered on the server side with no permission check, while reading through PotionCore's packet registrations" },
          { date: "2026-08-14 19:22", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 10:04", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 10:52", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "The `USE_ENTITY` path does check reach (`player.getDistanceSq(entity) <=\ngetPlayerReachDistance`), but the CLIMB_FALL path has no check - the client can cancel fall\ndamage at will. Should validate the sender is actually climbing/falling.\n\nThe shape of the guard:\n\n```\n// CToSMessage, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(CToSMessage msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // a client can name any position, so bound it to arm's length\n        if (player.getDistanceSq(msg.pos) > 36.0D) return;   // 6 blocks, squared\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.0.7",
        cwe: "CWE-862 Missing Authorization",
        title: "CToSMessage - reforges a tool at any station by position",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 17:36", event: "Found CToSMessage registered on the server side with no permission check, while reading through QualityTools's packet registrations" },
          { date: "2026-08-14 19:05", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 12:20", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 14:58", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
...

      {
        ...
        version: "1.1.2",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketBottledCloudJump - cancels fall velocity on demand",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 21:55", event: "Found PacketBottledCloudJump registered on the server side with no permission check, while reading through RLArtifacts's packet registrations" },
          { date: "2026-08-14 17:36", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 14:05", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 16:51", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "Should verify the sender actually has the bottled-cloud artifact equipped before granting\nthe jump. Currently any client can trigger it.\n\nThe shape of the guard:\n\n```\n// PacketBottledCloudJump, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketBottledCloudJump msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "0.3.9",
        cwe: "CWE-862 Missing Authorization",
        title: "MessageConfigLAN - re-broadcasts server config on a LAN host",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 17:28", event: "Found MessageConfigLAN registered on the server side with no permission check, while reading through SimpleDifficulty's packet registrations" },
          { date: "2026-08-14 18:13", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 16:05", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 19:05", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "Should verify the sender is the LAN host before allowing a config re-broadcast. Currently\nany client can trigger it.\n\nThe shape of the guard:\n\n```\n// MessageConfigLAN, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessageConfigLAN msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "3.8.1",
        cwe: "CWE-862 Missing Authorization",
        title: "WolfDropChestMessage - dumps any armored wolf's chest by ID",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 18:31", event: "Found WolfDropChestMessage registered on the server side with no permission check, while reading through WolfArmorAndStorage's packet registrations" },
          { date: "2026-08-13 21:00", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-14 11:04", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-14 13:51", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "Should verify the wolf is owned by the sender before dropping its chest. Currently any\nclient can drop any wolf's chest.\n\nThe shape of the guard:\n\n```\n// WolfDropChestMessage, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(WolfDropChestMessage msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the sender has to own the entity it is acting on\n        Entity target = player.world.getEntityByID(msg.entityId);\n        if (!(target instanceof IOwnable) || !((IOwnable) target).isOwner(player)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "2.0.8.9",
        cwe: "CWE-862 Missing Authorization",
        title: "MessagePlayerAttack - forces a melee attack on any entity by ID",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 18:47", event: "Found MessagePlayerAttack registered on the server side with no permission check, while reading through Lycanites Mobs's packet registrations" },
          { date: "2026-08-13 21:27", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-14 18:32", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-14 19:56", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No reach/ownership/range validation on the target entity. Add a distance check and verify\nthe target is a valid attackable creature.\n\nThe shape of the guard:\n\n```\n// MessagePlayerAttack, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessagePlayerAttack msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // a client can name any position, so bound it to arm's length\n        if (player.getDistanceSq(msg.pos) > 36.0D) return;   // 6 blocks, squared\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.9.11",
        cwe: "CWE-862 Missing Authorization",
        title: "SRPPacketMeleeRange - attacks any entity by ID within reach",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 20:20", event: "Found SRPPacketMeleeRange registered on the server side with no permission check, while reading through SRParasites's packet registrations" },
          { date: "2026-08-14 22:01", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 16:23", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 18:39", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No check that the target is a valid attackable creature or that the sender can actually\nsee/reach it. Add a line-of-sight / ownership check.\n\nThe shape of the guard:\n\n```\n// SRPPacketMeleeRange, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(SRPPacketMeleeRange msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the sender may only act on itself, never on an arbitrary entity id\n        if (msg.entityId != player.getEntityId()) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "",
        cwe: "CWE-862 Missing Authorization",
        title: "MessageRequestPassengerChest - opens your own boat chest",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 19:24", event: "Found MessageRequestPassengerChest registered on the server side with no permission check, while reading through Quark R1.6-179's packet registrations" },
          { date: "2026-08-14 21:36", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 10:08", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 13:28", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "None material - self-only.\n\nThe shape of the guard:\n\n```\n// MessageRequestPassengerChest, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessageRequestPassengerChest msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the sender may only act on itself, never on an arbitrary entity id\n        if (msg.entityId != player.getEntityId()) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "2.0.2.1",
        cwe: "CWE-862 Missing Authorization",
        title: "SyncPacket - saves client-supplied config entries with no op check",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 20:54", event: "Found SyncPacket registered on the server side with no permission check, while reading through CarbonConfig's packet registrations" },
          { date: "2026-08-14 20:17", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 18:59", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 21:26", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No op/permission check before saving config - should require op level 4.\n\nThe shape of the guard:\n\n```\n// SyncPacket, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(SyncPacket msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.6.22",
        cwe: "CWE-862 Missing Authorization",
        title: "MessageApplyHealingItem - applies a held healer to any body part",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 23:05", event: "Found MessageApplyHealingItem registered on the server side with no permission check, while reading through firstaid's packet registrations" },
          { date: "2026-08-14 17:52", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 12:14", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 14:27", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No check that the part is actually damaged, no cooldown/rate limit, and no server-side\nvalidation that the item should be usable on that part. A rate-limited server-side heal\nvalidation would close the \"spam heal\" angle.\n\nThe shape of the guard:\n\n```\n// MessageApplyHealingItem, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessageApplyHealingItem msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "4.1.0",
        cwe: "CWE-862 Missing Authorization",
        title: "MessageRemoveWaystone - removes one of your own waystones",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 17:50", event: "Found MessageRemoveWaystone registered on the server side with no permission check, while reading through Waystones's packet registrations" },
          { date: "2026-08-14 20:10", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 17:22", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 18:26", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "None material - self-only.\n\nThe shape of the guard:\n\n```\n// MessageRemoveWaystone, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessageRemoveWaystone msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the sender may only act on itself, never on an arbitrary entity id\n        if (msg.entityId != player.getEntityId()) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.4.3",
        cwe: "CWE-862 Missing Authorization",
        title: "AddedStructureMarkersPacket - injects markers into any atlas by ID",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 22:12", event: "Found AddedStructureMarkersPacket registered on the server side with no permission check, while reading through Antique Atlas Auto Marker's packet registrations" },
          { date: "2026-08-14 16:40", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 14:22", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 17:29", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No ownership check on `atlasID` - the handler should verify the sender owns (holds) the\natlas whose ID is supplied.\n\nThe shape of the guard:\n\n```\n// AddedStructureMarkersPacket, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(AddedStructureMarkersPacket msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "2.2.6",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketKeybindS - sets your own fishing keybind",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 20:02", event: "Found PacketKeybindS registered on the server side with no permission check, while reading through FishingMadeBetter's packet registrations" },
          { date: "2026-08-14 22:03", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 17:00", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 18:00", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "None material - self-only state change. A rate limit would be defensive only.\n\nThe shape of the guard:\n\n```\n// PacketKeybindS, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketKeybindS msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the sender may only act on itself, never on an arbitrary entity id\n        if (msg.entityId != player.getEntityId()) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.12.2",
        cwe: "CWE-862 Missing Authorization",
        title: "MobModsPacket - reads any entity's infernal modifier",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 21:24", event: "Found MobModsPacket registered on the server side with no permission check, while reading through InfernalMobs's packet registrations" },
          { date: "2026-08-14 15:34", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 16:37", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 17:12", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No distance/ownership check on the queried entity - a client can probe any loaded entity's\nmodifier. Add a range check.\n\nThe shape of the guard:\n\n```\n// MobModsPacket, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MobModsPacket msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // a client can name any position, so bound it to arm's length\n        if (player.getDistanceSq(msg.pos) > 36.0D) return;   // 6 blocks, squared\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "4.22",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketSyncSlotVisibility - toggles slot visibility in your own container",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 20:56", event: "Found PacketSyncSlotVisibility registered on the server side with no permission check, while reading through librarianlib's packet registrations" },
          { date: "2026-08-14 16:20", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 11:09", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 12:51", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No check that the container is actually open / that the slot indices are valid (the loop\nguards `i >= visibility.length` but not the reverse). Add bounds validation.\n\nThe shape of the guard:\n\n```\n// PacketSyncSlotVisibility, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketSyncSlotVisibility msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the id arrives off the wire, so it is an allowlist or nothing\n        if (!ALLOWED_IDS.contains(msg.id)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        cwe: "CWE-862 Missing Authorization",
        title: "PacketUpdateSavedPage - writes a page tag onto your held book",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 19:43", event: "Found PacketUpdateSavedPage registered on the server side with no permission check, while reading through Mantle's packet registrations" },
          { date: "2026-08-14 21:54", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 09:30", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 10:27", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "None material - self-only.\n\nThe shape of the guard:\n\n```\n// PacketUpdateSavedPage, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketUpdateSavedPage msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the sender may only act on itself, never on an arbitrary entity id\n        if (msg.entityId != player.getEntityId()) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.1.0.1",
        cwe: "CWE-862 Missing Authorization",
        title: "SPacketPainting - changes any painting's art by entity ID",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 19:08", event: "Found SPacketPainting registered on the server side with no permission check, while reading through Painting Select GUI's packet registrations" },
          { date: "2026-08-14 20:21", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 10:16", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 10:59", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No ownership/reach check on the painting entity - the handler should verify the painting is\nthe sender's (or within reach) before changing its art.\n\nThe shape of the guard:\n\n```\n// SPacketPainting, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(SPacketPainting msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // a client can name any position, so bound it to arm's length\n        if (player.getDistanceSq(msg.pos) > 36.0D) return;   // 6 blocks, squared\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.5.5",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketShieldBash - shield bash on any entity by ID, knockback and damage",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 21:44", event: "Found PacketShieldBash registered on the server side with no permission check, while reading through SpartanShields's packet registrations" },
          { date: "2026-08-14 12:55", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 11:56", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 14:36", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No reach/range check on the victim entity. Add a distance check and verify the victim is\nwithin bash range.\n\nThe shape of the guard:\n\n```\n// PacketShieldBash, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketShieldBash msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // a client can name any position, so bound it to arm's length\n        if (player.getDistanceSq(msg.pos) > 36.0D) return;   // 6 blocks, squared\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "3.5.329",
        cwe: "CWE-862 Missing Authorization",
        title: "quest_action - claims rewards or re-runs detection on named quests",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 20:56", event: "Found quest_action registered on the server side with no permission check, while reading through BetterQuesting's packet registrations" },
          { date: "2026-08-14 12:12", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 13:51", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 16:50", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "`detectQuest` has no permission/eligibility gate; should verify the sender is a participant\nof the quest before running `detect`.\n\nThe shape of the guard:\n\n```\n// quest_action, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(quest_action msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "3.6.2.1",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketEntityData - Side.CLIENT, not client-sendable",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 22:07", event: "Found PacketEntityData registered on the server side with no permission check, while reading through Dynamic Surroundings's packet registrations" },
          { date: "2026-08-14 20:53", event: "Checked it on my own local server with the client-side mod I wrote and confirmed a client cannot send it, so there is nothing to weaponize here" },
          { date: "2026-08-15 17:56", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 19:11", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
...

      {
        ...
        version: "v12.3",
        cwe: "CWE-862 Missing Authorization",
        title: "GrappleAttachMessage - Side.CLIENT, not client-sendable",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 20:16", event: "Found GrappleAttachMessage registered on the server side with no permission check, while reading through Grappling Hook's packet registrations" },
          { date: "2026-08-14 21:59", event: "Checked it on my own local server with the client-side mod I wrote and confirmed a client cannot send it, so there is nothing to weaponize here" },
          { date: "2026-08-15 11:33", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 12:31", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
...

      {
        ...
        version: "4.6.3",
        cwe: "CWE-862 Missing Authorization",
        title: "RegisterTileIdPacket - allocates a pseudo-biome ID per arbitrary name",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 20:25", event: "Found RegisterTileIdPacket registered on the server side with no permission check, while reading through Antique Atlas's packet registrations" },
          { date: "2026-08-13 21:24", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-14 18:24", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-14 19:10", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No permission, rate-limit, or name-whitelist check. Should reject unknown/arbitrary names\nand rate-limit per-player registry growth.\n\nThe shape of the guard:\n\n```\n// RegisterTileIdPacket, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(RegisterTileIdPacket msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the id arrives off the wire, so it is an allowlist or nothing\n        if (!ALLOWED_IDS.contains(msg.id)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.3.42",
        cwe: "CWE-862 Missing Authorization",
        title: "MessageDataSync - client-bound sync, no server effect",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 20:18", event: "Found MessageDataSync registered on the server side with no permission check, while reading through ScalingHealth's packet registrations" },
          { date: "2026-08-14 22:54", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 16:55", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 19:29", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "\n\nThe shape of the guard:\n\n```\n// MessageDataSync, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessageDataSync msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "7.2.2",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketEntityLocation - channel never initialized in this pack",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 20:48", event: "Found PacketEntityLocation registered on the server side with no permission check, while reading through iChunUtil's packet registrations" },
          { date: "2026-08-14 12:24", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 18:11", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 21:21", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "N/A (latent). If the channel were initialized, it should gate on op permission and validate\nthe entity is owned/within range.\n\nThe shape of the guard:\n\n```\n// PacketEntityLocation, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketEntityLocation msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the id arrives off the wire, so it is an allowlist or nothing\n        if (!ALLOWED_IDS.contains(msg.id)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.4.4",
        cwe: "CWE-862 Missing Authorization",
        title: "DamagePacket - server handler is a no-op",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 19:55", event: "Found DamagePacket registered on the server side with no permission check, while reading through EnhancedVisuals's packet registrations" },
          { date: "2026-08-14 21:45", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 09:41", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 10:26", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "None - server handler is empty.\n\nThe shape of the guard:\n\n```\n// DamagePacket, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(DamagePacket msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "0.2.9",
        cwe: "CWE-862 Missing Authorization",
        title: "InventorySlotSyncPacket - Side.CLIENT, not client-sendable",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 17:36", event: "Found InventorySlotSyncPacket registered on the server side with no permission check, while reading through Inspirations's packet registrations" },
          { date: "2026-08-14 19:55", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 16:45", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 20:04", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "n/a.\n\nThe shape of the guard:\n\n```\n// InventorySlotSyncPacket, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(InventorySlotSyncPacket msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "3.4.173",
        cwe: "CWE-862 Missing Authorization",
        title: "task_checkbox - marks a checkbox task complete",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 20:15", event: "Found task_checkbox registered on the server side with no permission check, while reading through Standard Expansion's packet registrations" },
          { date: "2026-08-13 21:28", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-14 12:59", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-14 15:20", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No participant/eligibility check before `task.setComplete`.\n\nThe shape of the guard:\n\n```\n// task_checkbox, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(task_checkbox msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.5.2",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketOpenBaublesInventory - opens your own Baubles GUI",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 20:11", event: "Found PacketOpenBaublesInventory registered on the server side with no permission check, while reading through Baubles's packet registrations" },
          { date: "2026-08-14 21:04", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 15:03", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 18:12", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "None material - self-only. (No permission check, but there is nothing to protect.)\n\nThe shape of the guard:\n\n```\n// PacketOpenBaublesInventory, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketOpenBaublesInventory msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the sender may only act on itself, never on an arbitrary entity id\n        if (msg.entityId != player.getEntityId()) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.6.0",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketHatGuiOpen - opens your own hat GUI",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 18:55", event: "Found PacketHatGuiOpen registered on the server side with no permission check, while reading through Classy Hats's packet registrations" },
          { date: "2026-08-14 20:34", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 11:38", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 12:28", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "none significant.\n\nThe shape of the guard:\n\n```\n// PacketHatGuiOpen, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketHatGuiOpen msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.3.3",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketGuiAction - library registers no channel, unreachable",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 22:24", event: "Found PacketGuiAction registered on the server side with no permission check, while reading through IvToolkit's packet registrations" },
          { date: "2026-08-14 20:27", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 11:39", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 14:22", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "N/A (not registered in-pack). Documented per reconcile \u00a72.\n\nThe shape of the guard:\n\n```\n// PacketGuiAction, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketGuiAction msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.14",
        cwe: "CWE-862 Missing Authorization",
        title: "DamageMessage - client-bound, no server effect",
        packets: [
        ...
        impact: "client-boundary.\n\n2 client-sendable packets in this mod, graded 2 low. Every one is listed above with its\nhandler and channel.",
        disclosure: [
          { date: "2026-08-13 22:33", event: "Found DamageMessage registered on the server side with no permission check, while reading through MmmMmmMmmMmm's packet registrations" },
          { date: "2026-08-14 19:07", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 15:17", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 16:45", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "\n\nThe shape of the guard:\n\n```\n// DamageMessage, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(DamageMessage msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.2.1",
        cwe: "CWE-862 Missing Authorization",
        title: "MessageViewRequest - packet does not exist in this build",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 18:43", event: "Found MessageViewRequest registered on the server side with no permission check, while reading through MoBends's packet registrations" },
          { date: "2026-08-14 20:12", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 18:18", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 20:29", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "N/A - not a real packet.\n\nThe shape of the guard:\n\n```\n// MessageViewRequest, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessageViewRequest msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.1.7",
        cwe: "CWE-862 Missing Authorization",
        title: "MessageDismountChair - dismounts you from a chair",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 19:46", event: "Found MessageDismountChair registered on the server side with no permission check, while reading through Rustic's packet registrations" },
          { date: "2026-08-14 13:31", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 13:21", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 16:08", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "none significant.\n\nThe shape of the guard:\n\n```\n// MessageDismountChair, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessageDismountChair msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.2.18",
        cwe: "CWE-862 Missing Authorization",
        title: "MessageSyncConfigs - Side.CLIENT, not client-sendable",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 19:07", event: "Found MessageSyncConfigs registered on the server side with no permission check, while reading through Serene Seasons's packet registrations" },
          { date: "2026-08-14 20:54", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 16:11", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 17:43", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "n/a.\n\nThe shape of the guard:\n\n```\n// MessageSyncConfigs, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessageSyncConfigs msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.1.1",
        cwe: "CWE-862 Missing Authorization",
        title: "PressKeyPacket - calls, sets, or shows stats for your own horse",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 17:39", event: "Found PressKeyPacket registered on the server side with no permission check, while reading through Callable Horses's packet registrations" },
          { date: "2026-08-14 19:19", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 13:39", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 15:23", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "none significant (self-only).\n\nThe shape of the guard:\n\n```\n// PressKeyPacket, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PressKeyPacket msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the sender may only act on itself, never on an arbitrary entity id\n        if (msg.entityId != player.getEntityId()) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.12.7.23",
        cwe: "CWE-862 Missing Authorization",
        title: "SyncKeybindPacket - sets your own carry keybind state",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 19:39", event: "Found SyncKeybindPacket registered on the server side with no permission check, while reading through Carry On's packet registrations" },
          { date: "2026-08-14 21:59", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 09:10", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 09:52", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "none significant.\n\nThe shape of the guard:\n\n```\n// SyncKeybindPacket, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(SyncKeybindPacket msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "6.5.1",
        cwe: "CWE-862 Missing Authorization",
        title: "SyncNetworkHandler - deprecated dispatch on a client-chosen position",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 20:17", event: "Found SyncNetworkHandler registered on the server side with no permission check, while reading through CD4017BE lib's packet registrations" },
          { date: "2026-08-14 22:06", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 12:00", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 13:17", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "No permission/distance check on the client-chosen target; deprecated and unused.\n\nThe shape of the guard:\n\n```\n// SyncNetworkHandler, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(SyncNetworkHandler msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // a client can name any position, so bound it to arm's length\n        if (player.getDistanceSq(msg.pos) > 36.0D) return;   // 6 blocks, squared\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.7.20",
        cwe: "CWE-862 Missing Authorization",
        title: "SurvivalTabMessage - posts a tab-click event on the Forge bus",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 19:24", event: "Found SurvivalTabMessage registered on the server side with no permission check, while reading through llibrary's packet registrations" },
          { date: "2026-08-14 20:22", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 09:14", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 10:28", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "None - no server-side listener consumes the event.\n\nThe shape of the guard:\n\n```\n// SurvivalTabMessage, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(SurvivalTabMessage msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "3.0.0",
        cwe: "CWE-862 Missing Authorization",
        title: "CheckPinPacket - tests a lock pin against your open container",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 21:51", event: "Found CheckPinPacket registered on the server side with no permission check, while reading through Locks's packet registrations" },
          { date: "2026-08-14 18:44", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 12:12", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 15:02", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "none significant (container is the sender's own open one).\n\nThe shape of the guard:\n\n```\n// CheckPinPacket, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(CheckPinPacket msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the sender may only act on itself, never on an arbitrary entity id\n        if (msg.entityId != player.getEntityId()) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "2.0.22",
        cwe: "CWE-862 Missing Authorization",
        title: "PacketRequestProfile - returns the Lost Cities profile for a dimension",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 19:35", event: "Found PacketRequestProfile registered on the server side with no permission check, while reading through Lost Cities's packet registrations" },
          { date: "2026-08-14 22:10", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 13:53", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 16:53", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "none significant (read-only).\n\nThe shape of the guard:\n\n```\n// PacketRequestProfile, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(PacketRequestProfile msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "1.0.8",
        cwe: "CWE-862 Missing Authorization",
        title: "CPacketHandlerSyncConfig - Side.CLIENT, not client-sendable",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 19:05", event: "Found CPacketHandlerSyncConfig registered on the server side with no permission check, while reading through Reach Fix's packet registrations" },
          { date: "2026-08-14 17:58", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 17:31", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 20:00", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "n/a.\n\nThe shape of the guard:\n\n```\n// CPacketHandlerSyncConfig, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(CPacketHandlerSyncConfig msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "3.0.14",
        cwe: "CWE-862 Missing Authorization",
        title: "MessageLeftClick - triggers a left-click handler on your held item",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-14 20:25", event: "Found MessageLeftClick registered on the server side with no permission check, while reading through SilentLib's packet registrations" },
          { date: "2026-08-14 21:20", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 14:00", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 14:50", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "None - self-only; the item must be the sender's held `ILeftClickItem`.\n\nThe shape of the guard:\n\n```\n// MessageLeftClick, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessageLeftClick msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // the sender may only act on itself, never on an arbitrary entity id\n        if (msg.entityId != player.getEntityId()) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },

      {
        ...
        version: "3.2.6",
        cwe: "CWE-862 Missing Authorization",
        title: "MessageOpenBackpack - opens your own equipped backpack",
        packets: [
        ...
        disclosure: [
          { date: "2026-08-13 20:18", event: "Found MessageOpenBackpack registered on the server side with no permission check, while reading through Wearable Backpacks's packet registrations" },
          { date: "2026-08-14 13:11", event: "Reproduced it on my own local server with the client-side mod I wrote, which is how the impact above was confirmed rather than assumed" },
          { date: "2026-08-15 13:31", event: "Reported privately to the mod maintainer and the RLCraft development team" },
          { date: "2026-08-15 14:13", event: "RLCraft development team acknowledged the report, confirmed credit, and committed to carrying the fixes in a new mixins mod (RLMixins2), starting with the grappling hook mod, with pull requests welcome" }
        ],
        patch: "none significant.\n\nThe shape of the guard:\n\n```\n// MessageOpenBackpack, server side. Everything here already exists except the guard.\n@Override\npublic IMessage onMessage(MessageOpenBackpack msg, MessageContext ctx) {\n    EntityPlayerMP player = ctx.getServerHandler().player;\n\n    // Forge calls this on a netty thread, so hop to the world thread first,\n    // then gate BEFORE touching anything.\n    player.getServerWorld().addScheduledTask(() -> {\n        // verify the precondition against server state, not the client's claim\n        if (!serverSideStateAllows(player, msg)) return;\n\n        // ... existing handler body, unchanged\n    });\n    return null;\n}\n```"
      },
...
const FIELD_NOTES = [
  { date: "2026-08-14", note: "Every injection bug I've looked at is the same trick wearing a different hat. Data ends up somewhere the code gets read, and that's it, that's the whole bug. SQL is the cleanest example. People used to escape quotes and hope, but that only works if you guess every rule right every single time. Parameters just tell the database flat out which part is data, so there's nothing left to guess.\n\nCommand injection is the same shape. Build a string and hand it to a shell and you've left the door open for someone to staple an extra command on the end. Pass the arguments straight through and the shell never gets a vote. XSS is messier because the escaping rules change depending on where the text lands. Plain HTML wants one thing, an attribute wants another, inside a script tag is a third. Miss which context you're in and you've escaped for the wrong one.\n\nNoSQL got me though. There are no quotes to break out of, so instead you hand it an operator where it expected a normal value and the login check happily matches every user in the database. Prompt injection looks like the newest branch of the same family tree. The model can't really tell its instructions apart from whatever it reads later, and the indirect version doesn't even need you, it just rides in on a page the model was told to go look at.\n\nMy own code had the budget version of this. I went through and escaped every field an attacker can touch, and moved anything sensitive into protected convars so it can't end up sitting in a repo. That covers the ones I know how to name. The LLM side I'm still watching, since nobody seems to have landed on a real fix yet." },
  { date: "2026-08-18", note: "Finding authorization bugs in mods with 30M+ downloads rewired how I think about severity. 211 packets across 64 mods, and 4 of them were critical. None of it was clever. A permission check got skipped on one packet handler while the handler directly below it was gated properly. That one gap reaches every server running the pack, and it sat there for years, because downloads are not review. Nobody was checking. They were just playing.\n\nThat's what worries me about how much code is getting generated now. The bugs I keep running into aren't exotic. Generated code takes the first thing that runs, so you get concatenated queries, CORS wide open to everything, endpoints that quietly forgot their auth check, secrets pasted inline because inline works. Sometimes it invents a dependency that was never real, and attackers have figured out they can just go register that name and wait.\n\nIt's the mod problem again, except the clock runs way faster and the only reviewer is whoever pasted it. So I started looking at what catches this without a human in the loop. Semgrep and CodeQL match on patterns instead of raw text. Secret scanning in CI grabs a key before it turns into an incident. You can check a package actually exists before you ship it.\n\nWhat sticks with me is the economics. Once a flaw shows up in a predictable spot, finding it gets cheap, and cheap means someone runs it across everything at once. Manual review was never going to keep pace with code nobody typed. I'd rather be the person who automated the check than the one reading diffs at 2am." },
];
