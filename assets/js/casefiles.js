...
const FIELD_NOTES = [
  { date: "2026-08-14", note: "All these injection bugs seem to come down to the same thing really. Data gets treated like code somewhere and that is what lets it run. SQL is the clearest case. People used to try escaping but that only works if you guess the rules right every time. The better fix is just telling the database what is data and what is not through parameters.\n\nIt feels similar with command injection. Building a string for a shell leaves room for extra commands to sneak in. Passing the arguments directly removes the shell part so there is nothing left to trick. XSS works the same way except the escape rules change depending on where the data lands in the page. HTML text needs one thing and an attribute or script block needs another. That part gets messy if you miss the context.\n\nNoSQL was the one that surprised me. There are no quotes to break out of so instead you can send an operator where the code expected a plain value. The login check then matches everything. Prompt injection looks like the newest version of it. The model has no clean way to separate instructions from the stuff it pulls in later and indirect cases just come through a document or page it is told to read. Automated phishing kits do something close on the delivery side by making fake pages that forward the token right past some kinds of MFA.\n\nMy own code has the cheap version of the same problem. I ended up putting escape calls on every attacker field and keeping anything sensitive in protected convars so it does not end up in a repo. I think that covers most of it but I am not totally sure how the newer LLM cases will settle yet." },
  { date: "2026-08-18", note: "Finding real authorization bugs in Minecraft mods that had millions of downloads changed how I think about severity. Nobody missed something clever on purpose. A permission check just got skipped on one packet handler while the one next to it was properly gated. That single gap ends up hitting every server running the pack. It was a small thing with a huge reach and it stayed around for years since downloads do not equal review.\n\nThat got me thinking about how much code is written by AI tools now and what they tend to get wrong when it scales. The issues I keep seeing are not rare or fancy. Generated code often goes for whatever works first so you end up with string concatenated queries or CORS that is too open. Endpoints that should have checks do not and secrets get left inline because it runs right away. Dependencies sometimes get made up entirely and attackers are starting to register those names on purpose.\n\nIt feels like the same problem from the mods except it happens way faster and the only review is whoever pastes the result. I started looking at ways to catch this automatically. Tools like Semgrep and CodeQL can scan for patterns instead of just text. Secret scanning in CI helps catch a key before it becomes a real problem and there are checks that make sure a package actually exists before shipping.\n\nThe economics for attackers seem important here. When a flaw type shows up predictably the scanning to find it becomes cheap and can run at scale. Defenders probably need to match that with automation since manual review does not keep up with code that was not written by hand. It is not clear how much of this gets fixed early on." }
];
